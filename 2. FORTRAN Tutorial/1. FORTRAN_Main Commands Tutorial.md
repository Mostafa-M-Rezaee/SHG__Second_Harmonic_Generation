...
General Structure is suggested for writing programs.
...
Note 3: Usually lines are written in the data file to guide the user. So that these lines are not read by the main program and the program skips them, it is enough to write the following command once for each explanatory line.
...
To reset the file pointer to the beginning of a file so that its contents can be read again from the start, use the following command.:
...
## Method 1: Defining a one-sentence function
...
## Method 2: Defining a function using the Function command
